...
function elfRunNextInst() {
  ...
  for (var i = 1; ; i++) {
    ...
    if (RISCV.pc == (0x80152b58 | 0)) {
      if (readTest.length) {
        ...
        RISCV.priv_reg[PCR['CSR_FROMHOST']['num']] = new Long(
          0x100 | (readTest.shift().charCodeAt(0) & 0xff),
          0x01000000
        )
        RISCV.priv_reg[PCR['CSR_MSTATUS']['num']] =
          RISCV.priv_reg[PCR['CSR_MSTATUS']['num']] | 0x40000000
        ...
        if (tryCount == stopCount) {
          ...
          if (lastCharWritten == 0x1) {
            lastCharWritten = 0x0
            RISCV.priv_reg[PCR['CSR_COMPARE']['num']] =
              RISCV.priv_reg[PCR['CSR_MCYCLE']['num']] + 100000
            stopCount = 200000
          } else {
            stopCount = 10000
            return false
    ...
    if (RISCV.priv_reg[0x506] == RISCV.priv_reg[0x507]) {
      if ((RISCV.priv_reg[PCR['CSR_MSTATUS']['num']] & SR['SR_EI']) != 0x0) {
        // interrupts are enabled
        if (
          ((RISCV.priv_reg[PCR['CSR_MSTATUS']['num']] >>> 16) & 0x80) !=
          0x0
        ) {
          // timer interrupt is enabled
          // set IP bit for timer interrupt
          RISCV.priv_reg[PCR['CSR_MSTATUS']['num']] =
            RISCV.priv_reg[PCR['CSR_MSTATUS']['num']] | 0x80000000
